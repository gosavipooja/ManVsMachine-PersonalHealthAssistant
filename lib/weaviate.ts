...
import { WeaviateDocument } from '@/types';

class WeaviateService {
  private client: WeaviateClient;
  private isInitialized = false;
  private className = 'HealthData';
          ...
          .withClass({
            ...
            properties: [
              ...
              },
              {
                name: 'type',
                dataType: ['text'],
                description: 'Type of document (log, profile, coaching)',
              },
              {
                name: 'habitId',
                dataType: ['text'],
  ...
  async addDocument(document: WeaviateDocument) {
  ...
  async getDocumentsByType(type: string, userId?: string, limit: number = 10) {
    await this.initialize();
  ...
  async updateDocument(id: string, document: Partial<WeaviateDocument>) {
    await this.initialize();
    
    try {
      // Weaviate doesn't have direct update, so we delete and re-add
      await this.deleteDocument(id);
      await this.addDocument(document as WeaviateDocument);
    } catch (error) {
  ...
  async getSimilarLogs(habitId: string, userId: string, limit: number = 5) {
    await this.initialize();
  ...
  async getCoachingHistory(userId: string, limit: number = 10) {
    await this.initialize();
